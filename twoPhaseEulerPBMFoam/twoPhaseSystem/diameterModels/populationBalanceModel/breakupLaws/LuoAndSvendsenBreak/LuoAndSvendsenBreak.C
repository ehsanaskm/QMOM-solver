...
:
    ...
    k_(dictLuoAndSvendsenBreakup_.lookup("k"))
{}
...
//			mesh().objectRegistry::lookupObject<volScalarField>("epsilon.water");   			
  ...
  scalar fbv = 
					pow3(min(d1,d2)/max(d1,d2));
  scalar Cf = 
               pow(fbv,2./3.) + pow((1.-fbv), 2./3.) - 1.;
  
  scalar a = 
			k_.value()*beta_*max(vf).value()*Foam::pow(max(epsilon).value()/sqr(d1), 1./3.);
			
  scalar b = 
			12.0*Cf*sigma_.value()/
			(  
			       beta_*rhob_.value()*Foam::pow(max(epsilon).value(), 2./3.)
			                          *Foam::pow(d1, 5./3.) 
			     + SMALL
			);			
